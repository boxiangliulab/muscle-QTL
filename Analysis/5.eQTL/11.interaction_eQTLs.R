...
genotypes_encoded_adj <- genotypes_encoded_adj[, colnames(SAMs2_count_table_final)]

if (!all(colnames(genotypes_encoded_adj) == colnames(SAMs2_count_table_final))) {
  stop("Column names (sample order) in genotypes_encoded_adj and SAMs2_count_table_final do not match!")
}

eQTL_both_significant <- eQTL_summary_statistics_adj %>%
  group_by(eGene, SNP) %>%
  filter(n_distinct(condition) == 2) %>%
  ungroup()

prepare_data <- function(gene, snp) {
  expression <- as.numeric(SAMs2_count_table_final[gene, ])
  genotype <- as.numeric(genotypes_encoded_adj[snp, ])
  
  data <- data.frame(
    sample_id = colnames(SAMs2_count_table_final),
...
analyze_eQTL <- function(gene, snp) {
  ...
  model <- lm(expression ~ genotype + condition + genotype:condition + PC1 + PC2 + PC3, data = data)
...
results <- lapply(1:nrow(eQTL_both_significant), function(i) {
  tryCatch({
    analyze_eQTL(eQTL_both_significant$eGene[i], eQTL_both_significant$SNP[i])
  }, error = function(e) {
    message(paste("Error in analyzing", eQTL_both_significant$eGene[i], eQTL_both_significant$SNP[i], ":", e$message))
    return(data.frame(gene = eQTL_both_significant$eGene[i],
                      snp = eQTL_both_significant$SNP[i],
                      interaction_p = NA,
                      interaction_effect = NA))
  })
})
...
results_df$is_interaction_QTL <- results_df$fdr < 0.05 & abs(results_df$interaction_effect) > effect_size_threshold

write.csv(results_df, "eQTL_interaction_results.csv", row.names = FALSE)
interaction_QTLs <- results_df[results_df$is_interaction_QTL, ]
print(interaction_QTLs)
