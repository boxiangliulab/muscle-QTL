...
ggplot(data, aes(x = Population, y = AlleleFrequency, fill = Population)) +
  geom_bar(stat = "identity", color = "black") + # use geom_col() if you want to skip stating stat="identity"
  geom_hline(yintercept = threshold, linetype = "dashed", color = "red") +
  scale_fill_grey(start = 0.8, end = 0.2) + # This will create a grey scale fill similar to your plot
