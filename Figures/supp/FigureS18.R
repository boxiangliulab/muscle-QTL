...
sQTL_data <- read.table(gene_data_file, header = TRUE, stringsAsFactors = FALSE)
LD_matrix <- read.table(ld_matrix_file, header = TRUE, stringsAsFactors = FALSE)

sQTL_data$position <- as.numeric(sQTL_data$position)
sQTL_data$distance <- abs(sQTL_data$position - sQTL_data$position[sQTL_data$rs_id == target_rs_id])
sorted_data <- sQTL_data[order(sQTL_data$distance, sQTL_data$pval_nominal_sqtl), ]
...
for (i in 1:nrow(eQTL_data)) {
  LD_index <- findInterval(sQTL_data$LDscore[i], c(0, 0.2, 0.4, 0.6, 0.8, 1))
  sQTL_data$color[i] <- color_palette[LD_index]
}

# Plot
p1 <- ggplot(sQTL_data, aes(x = position / 1000000, y = -log(pval_nominal_sqtl, base = 10), fill = color)) +
